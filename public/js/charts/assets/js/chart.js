...
window.tooltips = function tooltips(){
	//toggle
	myBar.config.options.showAllTooltips = (!(myBar.config.options.showAllTooltips));

	console.info('tooltips:::::  '+ myBar.config.options.showAllTooltips+' -- '+ testx.config.tooltips);
	//myBar.config.options.showAllTooltips = false;

	myBar.config.options.showAllTooltips = testx.config.tooltips;
	myBar.update();
}
	...
	Chart.pluginService.register({
		beforeRender: function (chart) {
			...
			var test = chart.config.options.showAllTooltips;
			if (test) {
				...
				chart.config.data.datasets.forEach(function (dataset, i) {
					chart.getDatasetMeta(i).data.forEach(function (sector, j) {
						...
						chart.pluginTooltips.push(new Chart.Tooltip({
							...
							_data: chart.data,
							_options: chart.tooltip._options,
							_active: [sector]
						}, chart));
					});
				});
//console.info(chart);
		...
		afterDraw: function (chart, easing) {
			...
			//if (chart.config.options.showAllTooltips) {
				...
				//console.info(chart.pluginTooltips);
				//console.warn(myBar.data.datasets[0]._meta[0].controller.chart.pluginTooltips[0]._options.tooltips.callbacks);
				//myBar.data.datasets[0]._meta[0].controller.chart.tooltip._options.callbacks
				...
				Chart.helpers.each(chart.pluginTooltips, function (tooltip, index) {
					...
					////console.info(tooltip._active[0]._datasetIndex);
					/////console.info(tooltip._active[0]._chart.config.data.datasets[tooltip._active[0]._datasetIndex].label);
					//if (tooltip._active[0]._datasetIndex != window.testx.thisDataset._meta[0].data[0]._datasetIndex){

					var tDataset = window.testx.datasets[tooltip._active[0]._datasetIndex];


					if (tDataset.label === 'cross') {
						return true; //continue,  false = break
					}
					//console.info(tDataset.label+': ('+tooltip._active[0]._datasetIndex+') '+tDataset._meta[0].hidden);
					if (!(tDataset._meta[0].hidden)){
						tooltip.initialize();
...
//TODO
	...
	//TODO MOET chart db zijn

	//console.info(window.testx.config.options.scales.xAxes);
	//console.info(newChart.testx.chartbackground.text.align);
	...
	window.testx = new Vue({
		...
		watch:{
			...
			'config.tooltips': function (val, oldVal) {
				//console.log('new: %s, old: %s', val, oldVal)
				//console.info(this);
				///console.info(window.testx);
				this.updateTooltips();
			...
			'config.chartbackground.text.display': function (val, oldVal) {
				//console.log('new: %s, old: %s', val, oldVal)
				this.updateChart();
			},
			'config.chartbackground.text.align': function (val, oldVal) {
				//console.log('new: %s, old: %s', val, oldVal)
				this.updateChart();
			}
		...
		methods: {
			updateChart: function(event) {
				//console.info(window.testx.config.options.scales.xAxes.ticks.max);
				brancheAverage();
				window.myBar.update();
			},
			updateTooltips: function(event) {
				//console.info(window.testx.config.options.scales.xAxes.ticks.max);
				//brancheAverage();
				window.tooltips();
				window.myBar.update();
			...
			updateScale: function(event){
				if (window.testx.config.options.scales.yAxes == undefined){

				}
				else{
					//console.info(parseFloat(window.testx.config.options.scales.yAxes.ticks.max));
					window.myBar.config.options.scales.yAxes[0].ticks.max = parseFloat(window.testx.config.options.scales.yAxes.ticks.max);
				}
				if (window.testx.config.options.scales.xAxes == undefined){

				}
				else{
					//console.info(parseFloat(window.testx.config.options.scales.xAxes.ticks.max));
					window.myBar.config.options.scales.xAxes[0].ticks.max = parseFloat(window.testx.config.options.scales.xAxes.ticks.max);
				}

				if (window.testx.config.options.elements == undefined){

				}
				else{
					//console.info(parseFloat(window.testx.config.options.scales.xAxes.ticks.max));
					window.myBar.config.options.elements.point.radius = parseFloat(window.testx.config.options.elements.point.radius);
					window.myBar.config.options.elements.point.hoverRadius = parseFloat(window.testx.config.options.elements.point.radius);
	...
	//alert(window.testx.config.options.scales.xAxes.ticks.max);
	//console.warn(window.testx.config.options.scales);
	//console.info(window.testx.config.options.scales.ticks);
	...
	//window.testx.config.options.scales.xAxes.ticks.max = 9;
	var config = {
		type: 'line',
		...
		options: {
			...
			elements:{
				...
				point:{
					backgroundColor: 'red',
					hoverRadius:window.testx.config.options.elements.point.radius,
					radius:window.testx.config.options.elements.point.radius,
			...
			animation: {
				...
				onComplete: function(animation) {
					...
					//window.myBar.update();
					//var ccc = myBar.data.datasets[0];
					//myBar.showTooltip(ccc,true);
			...
			legend: {
				...
				onClick: function(event, legendItem) {
					...
					//tooltips(legendItem.datasetIndex);
			...
			scales: {
				yAxes: [{
					...
					ticks: {
						//max: 11, //window.testx.config.options.scales.xAxes.ticks.max,
						max: window.testx.config.options.scales.yAxes.ticks.max,
				...
				xAxes: [{
					...
					position: 'bottom',
					ticks: {
						max: window.testx.config.options.scales.xAxes.ticks.max,
						stepSize: 0.2,
						// Create scientific notation labels
						beginAtZero: true,
						callback: function(value, index, values) {
	...
	window.initChart = function initChart(id){
		getConfig(id);
...
*/
	...
	window.loadChartData = function loadChartData(chart){
		...
		//console.info(chartAsObject.config.datasets[0]);
		window.testx.thisDataset = null;
		...
		//console.info(window.dirtyInit);
		//if window.myBar ==
		if (window.dirtyInit){
			window.myBar = new Chart(ctx, config);
			window.testx.updateScale();
			window.dirtyInit = false;
		}
		else{
			//window.myBar.clear();
			//window.myBar = new Chart(ctx, config);
...
//		console.info(window.testx.config.target.crossIndex);
	...
	$('#removeDataset').click(function(evt) {
		...
		//console.warn(evt);
//		console.info('point+points[0]'+points[0]._datasetIndex);
		...
		//testIndex = Object.keys(window.testx.thisDataset._meta).length-1;
		...
		window.myBar.config.data.datasets.splice(activeIndex, 1);
	...
	window.brancheAverage = function brancheAverage(){
		var brancheAverageVP =  window.testx.config.brancheAverage.crossIndex[1];
		var brancheAverageMF =  window.testx.config.brancheAverage.crossIndex[0];
		//console.info('brancheAverageVP'+brancheAverageVP);
		//console.info('brancheAverageMF'+brancheAverageMF);
		var brancheAverageVPTotal = 0;
		var brancheAverageMFTotal = 0;
		var aantalAfdelingen = (window.testx.datasets.length-1);
		//console.info('aantalAfdelingen' +aantalAfdelingen);
		for (var i = 0;i<(aantalAfdelingen);i++){
			//console.info(i+' '+window.testx.datasets[i].label+' - '+window.testx.datasets[i].data[0].y+' - '+window.testx.datasets[i].data[0].x);


			//VP
			brancheAverageVPTotal += parseFloat(window.testx.datasets[i].data[0].y);

			//MF
			//window.testx.datasets[0].data[0].x
			brancheAverageMFTotal += parseFloat(window.testx.datasets[i].data[0].x);
		}

		brancheAverageVP = parseFloat((brancheAverageVPTotal/aantalAfdelingen)).toFixed(1);
		brancheAverageMF = parseFloat((brancheAverageMFTotal/aantalAfdelingen)).toFixed(1);

		//window.testx.config.brancheAverage.crossIndex[1] = brancheAverageVP;
		//window.testx.config.brancheAverage.crossIndex[0] = brancheAverageMF;

		//window.testx.config.brancheAverage.crossIndex = [3.1,5.6];
		window.testx.config.brancheAverage.crossIndex = [brancheAverageMF,brancheAverageVP];

		console.info('brancheAverage');
		return true;
		return [brancheAverageMF,brancheAverageVP];

		return brancheAverageVPTotal+' - '+brancheAverageMFTotal;
	}
...
window.newctx = ctx;
console.info(ctx);
	window.ctx1 = new C2S(500,500); //width, height of your desired svg file
	console.info(ctx1);
	//ctx1.canvas = ctx.canvas;
//do your normal canvas stuff:
	ctx1.fillStyle="red";
	ctx1.fillRect(100,100,100,100);
//ok lets serialize to SVG:
	window.myRectangle = ctx1.getSerializedSvg(true);
...
/*
 myBar.data.datasets[0]._meta[0].controller.chart.tooltip.draw();
 myBar.data.datasets[9]._meta[0].controller.index
 myBar.data.datasets[0]._meta[0].controller.chart.tooltip._chart.config.options.legend.display = false
 */